...
void render_board(RenderQueue *q, GLFWwindow* window, f_point scale) {
    ...
    U64* piece = q->board->held_bit_piece;
    if (piece) {
        // draw the held one last
        board_move_pos bp;
        double xd, yd;
...
bool get_board_pos(RenderQueue *q, double x, double y, board_move_pos *p) {
    ...
    if (!rect_contains_point((Rectangle) {start_x, start_y, q->new_box_len, q->new_box_len, false}, (f_point) {x, y})) {
       // fprintf(stderr, "oops, mouse clicked outside of board! at x: %f, y: %f\n", x, y);
        q->board->held_bit_piece = NULL;
        return false;
    }
    const float dl = q->new_box_len/8.0f;
    I8 col = floor((x-start_x)/dl);
    I8 row = floor((y-start_y)/dl);
    p->start_square = (row * 8) + col;
    p->end_square = p->start_square;
...
void mouse_button_callback(GLFWwindow* window, int button, int action, int mods) {
    ...
    if (button == GLFW_MOUSE_BUTTON_LEFT && action == GLFW_RELEASE) {
        ...
        if (held_piece) {
            ...
            board_move_pos p;
            if (get_board_pos(queue, xpos, ypos, &p) == true) {
                queue->board->held_piece_pos.end_square = p.start_square;
                move_piece_on_board(queue->board, queue->board->held_piece_pos);
                queue->board->held_bit_piece = NULL;
            }
            else {
                fprintf(stderr, "release callback, somehow get_board_pos returned false");
            }
        }
    }
}